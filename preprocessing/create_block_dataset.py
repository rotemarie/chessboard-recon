"""
Create 3x3 block dataset from game videos (games 2-7 only).
Each square is extracted as a 3x3 block (192x192 pixels) centered on that square.
"""
...
class BlockSquareExtractor:
    """Extract 3x3 blocks centered on each square from a warped board."""
    ...
    def extract_blocks(self, warped_board: np.ndarray) -> list:
        """
        Extract 64 3x3 blocks (192x192 each) from the warped board.
        Uses black border padding for edge squares.
        
        Returns:
            List of 64 blocks (from a8 to h1)
        """
        # Add padding of one square size on each side with black border
        ...
        blocks = []
        # Extract blocks in chess notation order (a8 to h1)
        for row in range(8):  # rows 8 to 1
            for col in range(8):  # columns a to h
                # Calculate center position of the square in the padded board
                ...
                # Extract 3x3 block centered on this square
                y_start = center_y - int(1.5 * self.square_size)
                x_start = center_x - int(1.5 * self.square_size)
                
                block = padded_board[
                    y_start : y_start + 3 * self.square_size,
                    x_start : x_start + 3 * self.square_size
                ]
...
def process_game(game_dir: Path, output_dir: Path, detector: BoardDetector, 
    ...
    """
    Process a single game: detect board, extract 3x3 blocks, save with labels.
    
    Args:
        game_dir: Path to game directory (e.g., data/per_frame/game2_per_frame)
        output_dir: Root output directory (preprocessed_data_blocks/train)
        detector: BoardDetector instance
        extractor: BlockSquareExtractor instance
    """
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc=f"  {game_name}"):
        ...
        # Save each block to its class directory
        ...
        for idx, block in enumerate(blocks):
            ...
            cv2.imwrite(str(output_file), block)
...
def main():
    """Process all tagged games (2-7) with 3x3 block extraction."""
    
    # Configuration
    DATA_ROOT = Path("/Users/rotemar/Documents/BGU/Intro to Deep Learning/final project/chessboard-recon/data/per_frame")
    OUTPUT_ROOT = Path("/Users/rotemar/Documents/BGU/Intro to Deep Learning/final project/chessboard-recon/preprocessed_data_blocks")
    BOARD_SIZE = 512
    ...
    extractor = BlockSquareExtractor(board_size=BOARD_SIZE)
    
    print(f"🎯 3x3 Block Extraction")
    print(f"   Board size: {BOARD_SIZE}x{BOARD_SIZE}")
    print(f"   Square size: {BOARD_SIZE//8}x{BOARD_SIZE//8}")
    print(f"   Block size: {3 * BOARD_SIZE//8}x{3 * BOARD_SIZE//8} (3x3 squares)")
